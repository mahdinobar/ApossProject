...
*	$Revision: 274 $
*
*	@example 	ECAT_3Ax_EPOS4-Test_csp.mc>>> modified for 2 axis and cst mode	//$B
*
*/
...
long main(void) {
	...
	sdkEpos4_SetupECatSdoParam(C_DRIVE_BUSID2, C_PDO_NUMBER, C_AXIS2_POLARITY, EPOS4_OP_CST );	//$B
	...
	{
		// setup EtherCAT bus module for cst mode	//$B
		sdkEpos4_SetupECatBusModule(axis[axisIndex], busIds[axisIndex], C_PDO_NUMBER, EPOS4_OP_CST);	//$B

		// setup virtual amplifier for cst mode	//$B
		sdkEpos4_SetupECatVirtAmp(axis[axisIndex], C_AXIS_MAX_RPM, EPOS4_OP_CST);	//$B

		// setup irtual counter for cst mode	//$B
		sdkEpos4_SetupECatVirtCntin(axis[axisIndex], EPOS4_OP_CST);	//$B
	...
	while(retval!=7)
	{
		retval.i[0] = 	sdkEpos4_AxisHomingStart(C_AXIS1, C_DRIVE_BUSID1, EPOS4_OP_CST, homingStateAx_0);	//$B
		retval.i[1] =  	sdkEpos4_AxisHomingStart(C_AXIS2, C_DRIVE_BUSID2, EPOS4_OP_CST, homingStateAx_1);	//$B
	}

	AxisControl(C_AXIS1,ON, C_AXIS2,ON);

	print("-----------------------------------------------------------");
	print("                Movement in CST Mode                       ");	//$B
	print("----------------------------------------------------------- \n");
